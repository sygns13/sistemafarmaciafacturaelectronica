...
import jym.ferreteria.datasource.DetalleSalidas;
import jym.ferreteria.datasource.SalidasLibresDataSource;
...
public class FrmReporteSalidaProductos extends javax.swing.JInternalFrame {
   ...
   private Controlador control = new Controlador();
    ...
    private void cargarDatos() {
        ...
        SalidasLibresDataSource dataSource = new SalidasLibresDataSource();
        ...
        if (idProducto == null) {
            if (txtFechaDesde.getDate() == null || txtFechaHasta.getDate() == null) {//sin fechas
                ...
                sql = "select r.idretiro, (@rownum:=@rownum+1) AS `posicion`, date_format(r.fecha,'%d/%m/%Y') as fecha, r.hora, concat('Und. ',pre.present,' ',p.nomproducto) as producto, l.nombre,'Unidad', r.cantreal, r.motivo, u.nomusr \n" +
...
" inner join presentacion pre on pre.idpresentacion= p.idpresentacion \n" +
" left join usuario u on u.idusuario=r.iduser "
                        + " where (r.`fecha` between '" + control.Formato_Amd(txtFechaDesde.getDate()) + "'and '" + control.Formato_Amd(txtFechaHasta.getDate()) + "')";
                fechas = "Salidas Libres Desde: " + ((JTextField) txtFechaDesde.getDateEditor()).getText() + " Hasta: " + ((JTextField) txtFechaHasta.getDateEditor()).getText();
...
" inner join produto p on p.idproduto=l.idproducto \n" +
" inner join presentacion pre on pre.idpresentacion= p.idpresentacion \n" +
" left join usuario u on u.idusuario=r.iduser where p.`idproduto`=" + idProducto;
...
" inner join presentacion pre on pre.idpresentacion= p.idpresentacion \n" +
" left join usuario u on u.idusuario=r.iduser where p.`idproduto`=" + idProducto + " "
                        + "AND (r.`fecha` between '" + control.Formato_Amd(txtFechaDesde.getDate()) + "'and '" + control.Formato_Amd(txtFechaHasta.getDate()) + "')";
                fechas = "Salidas Libres Desde: " + ((JTextField) txtFechaDesde.getDateEditor()).getText() + " Hasta: " + ((JTextField) txtFechaHasta.getDateEditor()).getText();
        ...
        try {
            while (result.next()) {
                ...
                dataSource.addDetalleSalida(new DetalleSalidas(result.getString(1),result.getString(2),result.getString(3),result.getString(4),result.getString(5),result.getString(6),result.getString(7),result.getInt(8),result.getString(9),result.getString(10)));
            }
            control.showReportDialog("Listado de Salidas Libres", "reporteSalidasLibres", map, dataSource);
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Reporte de Salida de Productos Libres");
