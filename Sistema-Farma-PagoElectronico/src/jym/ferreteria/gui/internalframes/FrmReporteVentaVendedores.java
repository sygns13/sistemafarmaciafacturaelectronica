...
import jym.ferreteria.gui.dialogs.seleccionarUsuario;

public class FrmReporteVentaVendedores extends javax.swing.JInternalFrame {
    ...
    public FrmReporteVentaVendedores() {
        initComponents();
        userspeshal();
   ...
   // JOptionPane.showMessageDialog(null, idusu);
    ...
    private void seleccionarCliente() {
        if (seleccionarUsuario == null) {
            seleccionarUsuario = new seleccionarUsuario(null, true);
        }
        seleccionarUsuario.setLocationRelativeTo(null);
        seleccionarUsuario.setVisible(true);
        String[] datos = seleccionarUsuario.getDatos();
        usuario = datos[0];
...
"                CONCAT(t.nombre,' N° ', co.serie,' - ' ,co.`numero`) as comprobante,du.nom , (v.subTotalInafecto+v.subTotalAfecto) AS subTotal,v.igv,(v.subTotalInafecto+ v.subTotalAfecto + v.igv) AS total , ifnull(v.hora,'--') as hora, u.nomusr \n" +
"                FROM venta v left join usuario u on u.idusuario=v.idusuario LEFT JOIN comprobante co ON v.idComprobante=co.idComprobante \n" +
"                LEFT JOIN tipocomprobante t on co.`idTipoComprobante`=t.`idTipoComprobante` \n" +
"                left join datosusuario du on du.Usuario_idusuario=u.idUsuario where 1=1 "+andtipo+" ";
        String extra = "";
        String fechas = "";

        if (usuario == null) {//Si usuario vacío
            ...
            if (txtFechaDesde.getDate() == null || txtFechaDesde.getDate() == null) {//fecha vacía
                if (cboEstado.getSelectedIndex() < 0) {//todos los estados
                    if (cboTipoComprobante.getSelectedIndex() == 0) {//Todas los tipos de comprobantes
                        ...
                        extra = " AND u.`idUsuario`=" + usuario + " ORDER BY v.fecha";

                    } else {// cliente seleccionado, fecha vacía, Todos los estados,tipo de comprobante específico
                        extra = " AND u.`idUsuario`=" + usuario + "  AND t.idTipoComprobante=" + cboTipoComprobante.getSelectedIndex() + " ORDER BY v.fecha;";
                    }
                } else {//Con estado en específico
                    String estado = "Anulado";
                    if (cboEstado.getSelectedIndex() == 1) {
                    ...
                    if (cboTipoComprobante.getSelectedIndex() == 0) {//Todas los tipos de comprobantes
                        ...
                        extra = " AND u.`idUsuario`=" + usuario + " AND v.`estado`='" + estado + "' ORDER BY v.fecha";
                    } else {// cliente seleccionado, fecha vacía, estado en específico,tipo de comprobante específico
                        extra = " AND u.`idUsuario`=" + usuario + " AND t.idTipoComprobante=" + cboTipoComprobante.getSelectedIndex() + " AND v.`estado`='" + estado + "' ORDER BY v.fecha;";
                    }
                }
            } else {//Con intervalo de fechas
                if (cboEstado.getSelectedIndex() < 0) {//todos los estados
                    if (cboTipoComprobante.getSelectedIndex() == 0) {//Todas los tipos de comprobantes
                        // cliente seleccionado, fecha vacía, Todos los estados, todos los tipos de comprobantes
                        extra = " AND u.`idUsuario`=" + usuario + " AND (v.`fecha` BETWEEN '" + control.Formato_Amd(txtFechaDesde.getDate()) + "' AND '" + control.Formato_Amd(txtFechaHasta.getDate()) + "') ORDER BY v.fecha";
                    } else {// cliente seleccionado, fecha vacía, Todos los estados,tipo de comprobante específico
                        extra = " AND u.`idUsuario`=" + usuario + " AND t.idTipoComprobante=" + cboTipoComprobante.getSelectedIndex() + " ORDER BY v.fecha;";
                    }
                } else {//Con estado en específico
                    ...
                    if (cboTipoComprobante.getSelectedIndex() == 0) {//Todas los tipos de comprobantes
                        // cliente seleccionado, fecha vacía, estado en específico, todos los tipos de comprobantes
                        extra = " AND u.`idUsuario`=" + usuario + " AND (v.`fecha` BETWEEN '" + control.Formato_Amd(txtFechaDesde.getDate()) + "' AND '" + control.Formato_Amd(txtFechaHasta.getDate()) + "') AND v.`estado`='" + estado + "' ORDER BY v.fecha";
                    } else {// cliente seleccionado, fecha vacía, estado en específico,tipo de comprobante específico
                        extra = " AND u.`idUsuario`=" + usuario + " AND (v.`fecha` BETWEEN '" + control.Formato_Amd(txtFechaDesde.getDate()) + "' AND '" + control.Formato_Amd(txtFechaHasta.getDate()) + "') AND t.idTipoComprobante=" + cboTipoComprobante.getSelectedIndex() + " AND v.`estado`='" + estado + "' ORDER BY v.fecha;";
                    }
                }
                fechas = "Ventas Desde: " + ((JTextField) txtFechaDesde.getDateEditor()).getText() + " Hasta: " + ((JTextField) txtFechaHasta.getDateEditor()).getText();
        ...
        map.put("cliente", usuario == null ? "" : "Usuario: " + txtCliente.getText());
        ...
        try {
            ...
            control.showReportDialog("Listado por Vendedor", "reporteVentasVendedor", map, dataSource);
