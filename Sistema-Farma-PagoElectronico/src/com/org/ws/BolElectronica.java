...
public class BolElectronica {
    ...
    public static void actualhash(String docu_codigo, String hash){
    String sql="update cabecera set hashcode='"+hash+"' where docu_codigo='"+docu_codigo+"';";
    control.ejecutar(sql);
    }
    
    public static void actualEnviado(String docu_codigo, String cdr, String nota, String estatus, String enviar){
    String sql="update cabecera set cdr='"+cdr+"', cdr_nota='"+nota+"', docu_proce_status='"+estatus+"', docu_enviaws='"+enviar+"' where docu_codigo='"+docu_codigo+"';";
    control.ejecutar(sql);
    }
    
    public static String enviarSunat(String DigestValue, String ruc, String tipodoc, String numero, String docu_codigo){
    String res="";
    String resultado[] = new String [2];
    String unidadEnvio; // = Util.getPathZipFilesEnvio();
    String pathXMLFile;
    unidadEnvio = "d:\\\\Comprobantes-SUNAT\\";
    String firma=DigestValue;
    
    resultado = enviarZipASunat(unidadEnvio, ruc + "-"+tipodoc+"-" + numero + ".zip", ruc);
                    ...
                    if(resultado[0].equals("0")){
                        actualEnviado(docu_codigo,resultado[0], resultado[1],"L","E");
                        JOptionPane.showMessageDialog(null, "La venta fue transmitida a SUNAT exitosamente");
                    }else if(resultado[0].equals("E")){
                        actualEnviado(docu_codigo,resultado[0], resultado[1],"E","E");
                        JOptionPane.showMessageDialog(null, "La venta fue transmitida a SUNAT exitosamente, pero hay error en la venta enviada");
                    }else if(resultado[0].length()>0){
                        actualEnviado(docu_codigo,resultado[0], resultado[1],"A","E");
                        JOptionPane.showMessageDialog(null, "La venta fue transmitida a SUNAT exitosamente, pero fue anulada por criterio de SUNAT");
                    }else{
    ...
    public static String generarXMLZipiadoBoleta(DocumentoCabBean items, List<DocumentodetBean> detdocelec, List<Leyenda> leyendas, String docu_codigo) {
        ...
        try {
            ...
            if (items != null) {
                ...
                //======================crear XML =======================
                res = creaXml(items, detdocelec, leyendas, unidadEnvio);
                String firma=LecturaXML.obtenerDigestValue(pathXMLFile);
                actualhash(docu_codigo,firma);
                /*=======================ENVIO A SUNAT=============*/
                if (items.getDocu_enviaws().equals("S")) {
                    ...
                    resultado = enviarZipASunat(unidadEnvio, items.getEmpr_nroruc() + "-"+items.getDocu_tipodocumento()+"-" + items.getDocu_numero() + ".zip", items.getEmpr_nroruc());
                    System.out.println("El resultado 03 es "+resultado[0]);
                    System.out.println("El resultado 03 es "+resultado[1]);
...
//            javax.activation.DataHandler dataHandler = new javax.activation.DataHandler(fileDataSource);
            ...
            String pathRecepcion = "d:\\\\Comprobantes-SUNAT\\";
    ...
    private static String creaXml(DocumentoCabBean items, List<DocumentodetBean> detdocelec, List<Leyenda> leyendas, String unidadEnvio) {
        ...
        try {
           ...
           String keystoreType = "JKS";
            ...
            String pathXMLFile = unidadEnvio + items.getEmpr_nroruc() + "-"+items.getDocu_tipodocumento()+"-" + items.getDocu_numero() + ".xml";
          ...
          for (Leyenda leyenda : leyendas) {
           ...
           /* Calendar calendario = new GregorianCalendar();
            ...
            IssueTime.appendChild(doc.createTextNode(hora + ":" + minutos + ":" + segundos));*/

            Element InvoiceTypeCode = doc.createElementNS("", "cbc:InvoiceTypeCode");
             InvoiceTypeCode.setAttributeNS(null, "listID", "0101");
             InvoiceTypeCode.setIdAttributeNS(null, "listID", true);
